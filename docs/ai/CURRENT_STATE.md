...
2. Keep review filters, claiming/unclaiming, moderation actions, sticky moderator controls, post-preview scrolling, blurred-image behavior and score scrolling core-owned.
3. Use current `.reviewable-*` and `.review-item*` selectors with semantic `--er-*` tokens only; no theme JavaScript or template override.
...
- Added `stylesheets/moderation-workflows.scss` around current `.reviewable-container`, `.reviewable-filters`, `.review-item` BEM markup, post preview, timeline/note, score/history and moderator-action surfaces.
- Review cards use existing eRespawn semantic surfaces, borders, radii and restrained elevation in Light/Dark modes.
- Current core `.review-item` flex, sticky aside, z-index/dropdown and scrollable post-preview mechanics remain untouched; only presentation is enhanced.
- A minimal `.reviewable-item` compatibility bridge remains for optional integrations that still emit legacy markup.
...
- Added `spec/system/premium_moderation_workflows_spec.rb` for the native review queue using current `.review-item[data-reviewable-id]` and moderator-action selectors.
...
- Review filter layout, reviewable claiming, action dropdowns, dialogs/modals, sticky moderator controls, post-preview scrolling, image blurring and score-table scrolling remain core-owned.
