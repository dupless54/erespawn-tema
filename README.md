...
- Premium review-queue filters, reviewable cards, post previews, score/history surfaces and action chrome while retaining Discourse-owned moderation mechanics.
- Premium topic bulk-selection controls, selected-row states and bulk-action chrome while preserving native selection, shift-click, menu and modal behavior.
- Premium native Discourse Chat channel lists, navbar, messages, reactions, composer, drawer and thread-side-panel surfaces while preserving Chat-owned interaction and layout mechanics.
...
- Native composer, DModal, bookmark actions, private-message topic lists, review actions, bulk topic actions, Chat geometry and directory/container-query behavior remain owned by Discourse.
...
- Review filters, claiming, moderation actions, collapsed-post behavior, image blurring and score-table overflow remain core-owned.
- Topic bulk selection, row-click/shift-click selection, available actions, DMenu and bulk-action DModal behavior remain core-owned.
- Native Chat owns channel/message routing, message interactions, reactions, composer growth, uploads, safe-area handling, drawer/thread sizing, scrolling and z-index mechanics.
...
The repository also carries focused system smoke coverage for core features, topic discovery/reading, identity/search, auxiliary account surfaces, community utility routes, activity/static routes, the native moderation review queue, native topic bulk selection and native Discourse Chat.
